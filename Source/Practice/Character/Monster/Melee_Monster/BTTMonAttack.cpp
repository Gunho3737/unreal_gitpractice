...
EBTNodeResult::Type UBTTMonAttack::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	UObject* pObject = OwnerComp.GetBlackboardComponent()->GetValueAsObject(FName("Target"));
	if (IsValid(pObject))
	{
		AActor* pTarget = Cast<AActor>(pObject);
		FVector PlayerPos = pTarget->GetActorLocation();
		FRotator MonLookAt = UKismetMathLibrary::FindLookAtRotation(pMonster->GetActorLocation(), PlayerPos);
		pMonster->SetActorRotation(MonLookAt);
	}
